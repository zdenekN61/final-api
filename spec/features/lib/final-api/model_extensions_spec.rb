describe Build do
  describe '::search' do
    ...
    context 'exact match' do
      ...
      it 'filters by state Finished' do
        builds << FactoryGirl.create(:build_customized, stopped_by: user1, owner: user2, state: 'failed')
        results = Build.search('sts= Finished', 100, 0)
        results.each do |build|
          expect(%w(passed failed).include? build.state).to be true
        end
      end
    end
